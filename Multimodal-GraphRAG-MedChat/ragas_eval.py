...
try:
    ...
    from langchain_ollama import ChatOllama  # <--- UPDATED TO OLLAMA
    #from langchain_community.chat_models import ChatOllama  # <--- UPDATED TO OLLAMA
...
INTER_QUERY_DELAY_SEC = 5             # avoid rate-limiting on Groq + OpenAI
...
def _compute_ragas_metrics(
    ...
    Uses Local Ollama (mistral:7b) for LLM + HuggingFace for embeddings.
    """
    
    # 1. Import the necessary Ragas wrappers
    from ragas.llms import LangchainLLMWrapper
    from ragas.embeddings import LangchainEmbeddingsWrapper

    # 2. Use local Mistral 7B via Ollama
    llm = ChatOllama(
        model="mistral:7b", 
        temperature=0.1,
    )
    ...
    # 3. Wrap the Langchain objects in Ragas wrappers
    ...
    # 4. Pass the wrapped objects to evaluate()
    result = evaluate(
        dataset=ragas_ds,
        metrics=metrics,
        llm=ragas_llm,
