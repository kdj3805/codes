...
try:
    ...
    # NOTE: TruLens OpenAI provider replaced with Groq-compatible fallback
    from trulens.providers.openai import OpenAI as TruOpenAI  # kept for compatibility
...
def _init_trulens_feedbacks(groq_key: str):
    """
    ...
    provider = TruOpenAI(
        model_engine="openai/gpt-oss-120b",   # reuse same Groq-served model name
        api_key=groq_key,
    )
    groundedness = (
...
def main(argv: list[str] | None = None) -> None:
    ...
    # ── OpenAI key check ──────────────────────────────────────────────────────
    groq_key = os.getenv("GROQ_API_KEY", "")
    if (run_ragas or run_trulens) and not groq_key:
        log.warning(
            "GROQ_API_KEY not set — both RAGAS and TruLens use Groq as judge LLM.\n"
            "Set it in .env or export GROQ_API_KEY=sk-...  Metrics will be skipped."
        )
        run_ragas   = False
        run_trulens = False
