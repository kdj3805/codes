...
st.title("📄 Enterprise PDF Extraction + Adaptive Chunking + Embeddings")
...
if pdf:
    ...
    # EMBEDDING STEP
    # =========================
    with st.spinner("Generating embeddings (Ollama)…"):
        embedded_chunks = []
        for chunk in chunks:
    ...
    st.success("PDF processing + embeddings completed!")

    st.download_button(
        "Download final_chunks_with_embeddings.json",
        data=json.dumps(embedded_chunks, indent=2),
        file_name="final_chunks_with_embeddings.json"
    )

    st.subheader("Chunk Preview")
    st.json(embedded_chunks[:2])
